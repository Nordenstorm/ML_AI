...
class ScoreMatching():
    ...
    def train(self, data, noise_levels, learning_rate, iterations):
        ...
        for i in range(0,iterations):
            ...
            random_indices = torch.randperm(tensor_size[0])[:100]
            batch = data[random_indices]
            total_loss=self.denoising_loss(batch,noise_levels)
            total_loss.backward()
...
def main():
    ...
    coords = (1/4)*torch.distributions.gamma.Gamma(2.0, 1.0).sample(sample_shape=(200, 2))*(2 * torch.rand(200, 2) - 1)
